...
" Example :: EMIMDCABF4 - The system will contain: EMIM (two mols), DCA and BF4 (each one with one mole)"
  function folder(name::String,MM1,MM2)
    ...
    gmds   = ["gmd_an1.inp","gmd_an2.inp","gmd_sol.inp", "gmd_ca1.inp","gmd_ca2.inp"] 
      ...
      for r in replic
        ...
        for file in gmds
          ...
          for line in eachline(w)
            ...
            elseif occursin("CA2",line)
              println(new,replace(line, "CA2" => "NA", count=1))
            else
              println(new,line)
            end  
  ...
  folder("EMIMDCABF4",177.21, 109.81) 
