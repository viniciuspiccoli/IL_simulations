#!/bin/bash
  ...
  for run in 0.50 1.00 1.50 2.00 2.50 3.00 ; do
   ...
   # \cp -f work_dir/"$il"/"$run"/"$i"/simul.dcd                    work_dir/"$il_a"/"$run"/"$i"  
    ...
    \cp -f $work_dir/"$il"/"$run"/"$i"/md_prod.mdp                   $work_dir/"$il_a"/"$run"/"$i"
...
#  for run in  0.20 0.35 0.50 0.65 0.80 0.95 ; do
#   for i in {00..04}; do
#    
# 
#    
#
#      \cp -f /home/viniciusp/mestrado/"$il"/"$run"/"$i"/processed.xtc  ~/mestrado/"$il_a"/"$run"/"$i"/processed.xtc
#      \cp -f /home/viniciusp/mestrado/"$il"/"$run"/"$i"/processed.gro  ~/mestrado/"$il_a"/"$run"/"$i"/processed.gro
#      \cp -f /home/viniciusp/mestrado/"$il"/"$run"/"$i"/md_prod.gro  ~/mestrado/"$il_a"/"$run"/"$i"/md_prod.gro
#      \cp -f /home/viniciusp/mestrado/"$il"/"$run"/"$i"/md_prod.xtc  ~/mestrado/"$il_a"/"$run"/"$i"/md_prod.xtc
#
#    
#
#   done
